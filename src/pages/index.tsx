...
import {Form, InputNumber, Table, Typography, Input, DatePicker, Modal, Button, notification} from "antd";
...
const Home: NextPage<Props> = ({books}) => {
    ...
    const { data, isLoading } = useQuery<Book[], Error>('books', getBooks, { initialData: books });
    ...
    const mutationCreate = useMutation(createBook, {
        ...
        onError: (error: AxiosError) => {
            const target:string[] = error.response?.data?.target;
            if(target.length){
                target.forEach(e => {
                    notification['error']({
            ...
            render: (_: any, record: Book) => {
                ...
                return editable ? (
                    ...
                    <>
                        ...
                        <Typography.Link disabled={editingKey !== ''} onClick={() => mutationDelete.mutate(record.id)}>
                            Delete
                        </Typography.Link>
                    </>
                );
    ...
    const mergedColumns = columns.map(col => {
        ...
        return {
            ...
            onCell: (record: Book) => ({
                record,
                inputType: ['age', 'number_of_pages', 'number_of_chapters', 'number_of_published_books'].includes(col.dataIndex) ? 'number' : col.dataIndex === 'date_of_creation'? 'date' : 'text',
